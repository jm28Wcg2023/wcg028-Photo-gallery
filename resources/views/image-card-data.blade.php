                        ...
                        @if (Auth::check())
                            @if (Auth::user()->role == 0)
                                @if (in_array($image->id,$image_own_id))
                                    <a href="{{ route('download.image',$image->id) }}"  class="btn  d-flex w-0 justify-content-end download" id="download"><i class="bi bi-arrow-down"></i></a>
                                @else
                            ...
                            <form method="post" action="{{route('ImageDeleteAdmin',$image->id)}}">
                                ...
                                <a href="{{ route('download.image',$image->id) }}"  class="btn  d-flex w-0 justify-content-start download" id="download"><i class="bi bi-arrow-down"></i></a>
                            @endif
                        @else
